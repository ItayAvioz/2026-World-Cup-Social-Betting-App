-- Migration 44: fn_schedule_ai_summaries — switch from current_setting to vault
-- M5: Was using current_setting('app.edge_function_url') which returns NULL
--     Now reads from vault.decrypted_secrets (same pattern as fn_schedule_game_sync)
...
DECLARE
  ...
  v_srk       text;
BEGIN
  -- Read secrets from vault (same pattern as fn_schedule_game_sync)
